    ...
    return (
        <header className="mx-auto w-full h-20 px-2 sm:px-6 lg:px-10 flex justify-between items-center bg-stone-400 text-amber-950 font-semibold fixed z-10 top-0">
            <div className="lg:ml-15">
                <NavLink to={"/"}>
                    <p className="text-xl">
                        <span className="text-amber-300 pl-1 pb-1 font-bold border-dashed border-rose-50 border-1 border-r-0">Poké</span>
                        <span className="text-rose-50 pl-0.5 pr-1 pb-1 border-dashed border-amber-300 border-1 border-l-0">saga</span>
                    </p>
                </NavLink>
            </div>
            <nav className={`basis-0 hidden transform transition-transform ease duration-700 sm:block ${isOpen ? "translate-x-[-1.5%] opacity-100 w-screen bg-red-50 absolute top-0 text-center mt-20 py-3" : "translate-x-[150%] opacity-0 sm:opacity-100 sm:translate-x-0 sm:block sm:relative"}`}>
            ...
            <div className="mt-0.5 block sm:hidden max-w-[750px] text-2xl basis-[50%] text-end">
