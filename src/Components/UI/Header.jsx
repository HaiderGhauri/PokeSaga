...
export function Header() {
    ...
    return (
        <header className="mx-auto w-full h-20 px-2 sm:px-6 lg:px-10 flex justify-between items-center bg-stone-400 text-amber-950 font-semibold fixed z-10 top-0">
            ...
            <nav className={`transform transition-transform ease-in-out duration-700 sm:block ${isOpen ? "translate-x-[-1.35%]  w-screen bg-stone-400 absolute top-0 text-center mt-20 py-3" : "translate-x-full hidden sm:translate-x-0 sm:block sm:relative"}`}>
                ...
                <ul className={`flex justify-end gap-8 lg:pr-15 mt-1 ${isOpen ? "flex-col" : ""}`}>
                    <li><NavLink to={'/'} onClick={handleLinkClick} className={navLinkClass}>Home</NavLink></li>
                    <li><NavLink to={'explore'} onClick={handleLinkClick} className={navLinkClass}>Explore</NavLink></li>
                    <li><NavLink to={'about'} onClick={handleLinkClick} className={navLinkClass}>About</NavLink></li>
                    <li><NavLink to={'contact'} onClick={handleLinkClick} className={navLinkClass}>Contact</NavLink></li>
                </ul>
            </nav>
